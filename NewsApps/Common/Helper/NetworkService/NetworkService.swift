    ...
    public func request<R: Request>(_ request: R, completion: @escaping (Result<R.Response, Error>) -> Void) {
        ...
        guard var urlComponent = URLComponents(string: request.url) else {
            let error = NSError(
                ...
                code: 404,
                userInfo: [
                    NSLocalizedDescriptionKey: ErrorResponse.invalidEndpoint.rawValue
                ]
            )
            ...
            guard let response = response as? HTTPURLResponse, 200..<300 ~= response.statusCode else {
                return completion(.failure(
                    NSError(
                        ...
                        code: -1,
                        userInfo: [
                            NSLocalizedDescriptionKey: ErrorResponse.invalidResponse.rawValue
                        ]
                    )))
            }
            ...
            guard let data = data else {
                return completion(.failure(
                    NSError(
                        ...
                        code: -1,
                        userInfo: [
                            NSLocalizedDescriptionKey: ErrorResponse.noData.rawValue
                        ]
                    )
                ))
            }
